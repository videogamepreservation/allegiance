...
int main(int argc, char **argv) {
    int i,j,k;
    int gendim;
    int cubesize, diamondsize, seed=0, count;
    ...
    double norm, factor, randr, randmax, rangap, lensangle= 0, lensbase= 1;
    ...
    if (!numpoints) {
      if (isregular + islens + issphere + isaxis + isspiral + iswidth) {
...
void out1(double a) {
  ...
  if (isinteger) 
    fprintf(fp, "%d ", roundi( a));
  else
    fprintf(fp, qh_REAL_1, a);
} /* out1 */
...
void out2n( double a, double b) {
  ...
  if (isinteger)
    ...
    fprintf(fp, qh_REAL_2n, a, b);
} /* out2n */

void out3n( double a, double b, double c) { 

  if (isinteger)
    fprintf(fp, "%d %d %d\n", roundi(a), roundi(b), roundi(c));
  else
    fprintf(fp, qh_REAL_3n, a, b, c);
} /* out3n */
