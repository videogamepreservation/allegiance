...
%fedroot%\src\artwork\tools\pmesh.dx6\xtobm.exe %1.x
if errorlevel 1 goto error_xtobm

echo Filtering internal mesh format file
%fedroot%\src\artwork\tools\pmesh.dx6\fmesh.exe %1.m %filter1% %filter2% > %1.nice.m
if errorlevel 1 goto error_filtermesh
echo .

echo Simplifying mesh to a base mesh and a set of vertex splits
%fedroot%\src\artwork\tools\pmesh.dx6\meshsimp.exe %1.nice.m -prog %1.prog %simpopt1% %simpopt2% -simplify >%1.base.m
if errorlevel 1 goto error_simplify

%fedroot%\src\artwork\tools\pmesh.dx6\rlines.exe %1.prog >%1.rprog
if errorlevel 1 goto error_reverse
echo .

echo Converting Base Mesh and Vsplits to .X file
%fedroot%\src\artwork\tools\pmesh.dx6\fprog.exe -fbase %1.base.m -fprog %1.rprog -pm >%1.pm
if errorlevel 1 goto error_filterprog

%fedroot%\src\artwork\tools\pmesh.dx6\pmtopmx.exe %1.pm
if errorlevel 1 goto error_pmtopmx

echo .
%fedroot%\src\artwork\tools\pmesh.dx6\pmxtox.exe %1.pmx
