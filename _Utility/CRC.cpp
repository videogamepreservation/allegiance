...
unsigned update_crc (unsigned Crc, const void * pData, unsigned nSize)
{
    __asm
    {
        pushf
        ...
        mov     eax,Crc
        jecxz   crc_end
        xor     edx,edx             // upper 24bits stay zeroed
        mov     esi,pData
        mov     dl,al               // preload
...
crc_loop:
        ...
        dec     ecx
        mov     dl,al               // (interleaved)  preload for next pass
        jnz     crc_loop
...
Parameters:		data: pointer to data
				size: size of the data (can be zero)
...
int FileCRC(HANDLE hFile, char * szErrorMsg) 
{
   ...
   // scan memory and determine CRC
   int nCRC = MemoryCRC (buffer, size);
...
int FileCRC(const char *szFilename, char * szErrorMsg) 
{
   ...
   int nCRC = FileCRC(hFile, szErrorMsg);
